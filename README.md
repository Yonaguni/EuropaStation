...
This is a sourcecode-only release, so the next step is to compile the server files.  Open europa.dme by double-clicking it, open the Build menu, and click compile.  This'll take a little while, and if everything's done right you'll get a message like this:

    saving europa.dmb (DEBUG mode)
    
    europa.dmb - 0 errors, 0 warnings
...
Finally, to start the server, run Dream Daemon and enter the path to your compiled europa.dmb file.  Make sure to set the port to the one you  specified in the config.txt, and set the Security box to 'Trusted'.  Then press GO and the server should start up and be ready to join.
