...
/datum/extension/interactive/multitool/items/clothing/interact(var/obj/item/device/multitool/M, var/mob/user)
	...
	if(!u.has_sensors_somewhere() == -1)
		to_chat(user, "<span class='warning'>\The [u] doesn't have suit sensors.</span>")
		return
	u.set_sensors(u.has_sensors_somewhere() == SUIT_LOCKED_SENSORS ? SUIT_HAS_SENSORS : SUIT_LOCKED_SENSORS)
	to_chat(user, "<span class='notice'>You [u.has_sensors_somewhere() == SUIT_LOCKED_SENSORS ? "" : "un"]lock \the [u]'s sensor controls.</span>")
