...
/obj/machinery/mineral/mint/attack_hand(var/mob/user)
...
/obj/machinery/mineral/mint/Topic(href, href_list)
	...
	if(href_list["makeCoins"])
		...
		if (src.output)
			...
			var/obj/item/storage/moneybag/M
			switch(chosen)
				if(DEFAULT_WALL_MATERIAL)
					while(amt_iron > 0 && coinsToProduce > 0)
						if (locate(/obj/item/storage/moneybag,output.loc))
							M = locate(/obj/item/storage/moneybag,output.loc)
						else
							M = new/obj/item/storage/moneybag(output.loc)
