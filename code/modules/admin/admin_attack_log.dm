...
proc/log_and_message_admins(var/message as text, var/mob/user = usr, var/turf/location)
	...
	if(T)
		message = message + " [ADMIN_JUMP_LINK(T.x,T.y,T.z)]"
...
proc/admin_attack_log(var/mob/attacker, var/mob/victim, var/attacker_message, var/victim_message, var/admin_message)
	...
	msg_admin_attack("[key_name(attacker)] [admin_message] [key_name(victim)] (INTENT: [attacker? uppertext(attacker.a_intent) : "N/A"]) [ADMIN_JUMP_LINK(attacker.x,attacker.y,attacker.z)]")
