...
/obj/item/reagent_containers/syringe/afterattack(var/obj/target, var/mob/user, var/proximity)
	...
	switch(mode)
		...
		if(SYRINGE_INJECT)
			...
			if(!target.is_open_container() && !ismob(target) && !istype(target, /obj/item/reagent_containers/food) && !istype(target, /obj/item/clothing/mask/smokable/cigarette) && !istype(target, /obj/item/storage/fancy/cigarettes))
