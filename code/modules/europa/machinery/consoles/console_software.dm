/datum/console_module
	...
	var/visible = 1
	var/obj/machinery/europa/console/owner

/datum/console_module/New(var/obj/machinery/europa/console/new_owner)
	if(!istype(new_owner))
		qdel(src)
		return
	owner = new_owner
...
/datum/console_module/basic_interface/get_interface_data(var/mob/user)
	...
	if(owner.data_network)
		if(owner.data_network.connected_machines.len)
			for(var/obj/machinery/europa/thing in owner.data_network.connected_machines)
				dat += "- "
