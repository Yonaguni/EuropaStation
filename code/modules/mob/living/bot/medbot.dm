/mob/living/bot/medbot
	...
	var/treatment_brute = REAGENT_STYPTAZINE
...
/mob/living/bot/medbot/proc/valid_healing_target(var/mob/living/carbon/human/H)
	...
	if(reagent_glass && use_beaker && ((H.getBruteLoss() >= heal_threshold) || (H.getToxLoss() >= heal_threshold) || (H.getToxLoss() >= heal_threshold) || (H.getOxyLoss() >= (heal_threshold + 15))))
		for(var/rid in reagent_glass.reagents.volumes)
...
/obj/item/storage/firstaid/attackby(var/obj/item/robot_parts/S, var/mob/user)
...
/obj/item/firstaid_arm_assembly/attackby(var/obj/item/W, var/mob/user)
	..()
