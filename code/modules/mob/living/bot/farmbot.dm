...
/mob/living/bot/farmbot/UnarmedAttack(var/atom/A, var/proximity)
	...
	if(istype(A, /obj/machinery/portable_atmospherics/hydroponics))
		...
		switch(t)
			...
			if(FARMBOT_NUTRIMENT)
				...
				if(do_after(src, 30, A))
					...
					T.reagents.add_reagent(REAGENT_AMMONIA, 10)
		attacking = 0
		action = ""
		update_icons()
		T.update_icon()
		...
		while(do_after(src, 10) && tank.reagents.total_volume < tank.reagents.maximum_volume)
			tank.reagents.add_reagent(REAGENT_WATER, 10)
			if(prob(5))
				playsound(loc, 'sound/effects/slosh.ogg', 25, 1)
		attacking = 0
		action = ""
...
/mob/living/bot/farmbot/proc/process_tray(var/obj/machinery/portable_atmospherics/hydroponics/tray)
	...
	else if(refills_water && tray.waterlevel < 40 && !tray.reagents.has_reagent(REAGENT_WATER))
...
/obj/structure/reagent_dispensers/watertank/attackby(var/obj/item/robot_parts/S, var/mob/user)
...
/obj/item/farmbot_arm_assembly/attackby(var/obj/item/W, var/mob/user)
	..()
...
/obj/item/farmbot_arm_assembly/attack_hand(var/mob/user)
