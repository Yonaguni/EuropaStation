...
/datum/species
	...
	var/subdual_message = "slumps to the ground, too weak to continue fighting."
	var/subdual_message_self = "You're in too much pain to keep going..."

	// Environment tolerance/life processes vars.
	var/reagent_tag                                   //Used for metabolizing reagents.
	var/breath_pressure = 16                          // Minimum partial pressure safe for breathing, kPa
	var/breath_type = REAGENT_ID_OXYGEN                        // Non-oxygen gas breathed, if any.
	var/poison_type = REAGENT_ID_FUEL                          // Poisonous air.
	var/exhale_type = REAGENT_ID_CARBONDIOXIDE                // Exhaled gas type.
...
/datum/species/proc/equip_survival_gear(var/mob/living/human/H, var/extendedtank = 1)
	if(H.backbag == 1)
		if (extendedtank)	H.equip_to_backpack_or_del(new /obj/item/weapon/storage/box/engineer(H), slot_r_hand)
		else	H.equip_to_slot_or_del(new /obj/item/weapon/storage/box/survival(H), slot_r_hand)
	else
		if (extendedtank)	H.equip_to_backpack_or_del(new /obj/item/weapon/storage/box/engineer(H))
