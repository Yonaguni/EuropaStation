...
/datum/unarmed_attack/proc/apply_effects(var/mob/living/human/user,var/mob/living/human/target,var/armour,var/attack_damage,var/zone)
	...
	// This is mostly a proof of concept, proper inclusion will need
	// more balancing. 15% damage boost for being ROUGH AND TUMBLE.
	if(user.has_aspect("Brawler"))
		attack_damage = ceil(attack_damage*1.15)
	...
	if(attack_damage >= 5 && armour < 2 && !(target == user) && stun_chance <= attack_damage * 5) // 25% standard chance
		...
		if(!target.lying)
			target.visible_message("<span class='danger'>[target] [pick("slumps", "falls", "drops")] to the ground!</span>")
		else
