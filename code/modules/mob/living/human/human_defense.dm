...
/mob/living/human/hit_with_weapon(var/obj/item/I, mob/living/user, var/effective_force, var/hit_zone)
	var/obj/item/organ/external/affecting = get_organ(hit_zone)
	if(!affecting)
		return //should be prevented by attacked_with_item() but for sanity.

	var/use_verb = "attacked"
	if(I.attack_verb && I.attack_verb.len)
		use_verb = pick(I.attack_verb)
	visible_message("<span class='danger'>[src] has been [use_verb] in the [affecting.name] with [I.name] by [user]!</span>")
...
/mob/living/human/standard_weapon_hit_effects(obj/item/I, mob/living/user, var/effective_force, var/blocked, var/hit_zone)
	...
	if((I.damtype == BRUTE || I.damtype == SUBDUAL) && prob(25 + (effective_force * 2)))
		if(!stat)
...
/mob/living/human/proc/handle_suit_punctures(var/damtype, var/damage, var/def_zone)
	...
	if(!wear_suit) return
	if(!istype(wear_suit,/obj/item/clothing/suit)) return
	var/obj/item/clothing/suit/SS = wear_suit
