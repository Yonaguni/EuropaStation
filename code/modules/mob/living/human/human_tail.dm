...
// This is kinda a placeholder for fucking around with human mobs with multipart tails.
/mob/living/human
...
/obj/effect/tail/Crossed(var/atom/movable/crossing)
	...
	if(istype(crossing, /mob/living))
		...
		if(M.m_intent != "run") // Move intent constants when?
			return
		var/damage_taken
		if(istype(crossing, /mob/living/human))
			var/mob/living/human/H = crossing
...
/obj/effect/tail/New(var/mob/living/human/newowner)
	owner = newowner
	if(!istype(owner))
		owner = null
		qdel(src)
