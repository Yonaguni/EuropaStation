...
/mob/living/mecha/proc/refresh_hud()
	if(pilot && pilot.client)
		pilot.client.screen |= hud_elements
	if(client)
		client.screen |= hud_elements

/mob/living/mecha/instantiate_hud()
	var/i = 1
	for(var/hardpoint in hardpoints)
		var/obj/screen/movable/snap/mecha/hardpoint/H = new(src, hardpoint)
	...
	for(var/additional_hud in additional_hud_elements)
		var/obj/screen/movable/snap/mecha/M = PoolOrNew(additional_hud,src)
	...
	hud_open = locate(/obj/screen/movable/snap/mecha/toggle/hatch_open) in hud_elements
	refresh_hud()

/mob/living/mecha/handle_hud_icons()
...
/mob/living/mecha/handle_hud_icons_health()

	if(!hud_health)

		hud_health = PoolOrNew(/obj/screen/movable/snap/mecha/health,src)
		hud_health.screen_loc = "13:28,7:15"
		hud_elements |= hud_health

		// Debugging/placeholder, test another time.
		for(var/i=1;i<=5;i++)
			var/obj/screen/movable/snap/mecha/internal_system/IS = new(src)
	...
	if(!body.diagnostics || !body.diagnostics.is_functional())
