...
/obj/item/reagent_containers/pill/attack(mob/M as mob, mob/user as mob, def_zone)
	...
	if(M == user)
		...
		msg_admin_attack("[key_name_admin(user)] fed [key_name_admin(M)] with [name] Reagents: [contained] (INTENT: [uppertext(user.a_intent)]) [ADMIN_JUMP_LINK(user.x,user.y,user.z)]")
...
/obj/item/reagent_containers/pill/afterattack(obj/target, mob/user, proximity)
	...
	if(target.is_open_container() && target.reagents)
		...
		msg_admin_attack("[user.name] ([user.ckey]) spiked \a [target] with a pill. Reagents: [reagentlist()] (INTENT: [uppertext(user.a_intent)]) [ADMIN_JUMP_LINK(user.x,user.y,user.z)]")
...
/obj/item/reagent_containers/pill/antitoxin/initialize()
	..()
