...
/proc/load_admins()
		...
		for(var/line in Lines)
			...
			if(List.len)
				...
				if(ckey)
					...
					world.log << "Adding legacy admin [ckey] as \a [rank] with rights [rights]."
					insert_query.Execute(global_db)
					if(insert_query.ErrorMsg())
						world.log << "SQL ERROR: admin (1): [insert_query.ErrorMsg()]."

	var/database/query/query = new("SELECT * FROM admin;")
	query.Execute(global_db)
	if(query.ErrorMsg())
		world.log << "SQL ERROR: admin (2): [query.ErrorMsg()]."
	...
	while(query.NextRow())
		...
		world.log << "Created admin datum for [ckey] as \a [rank] with rights [rights]."
		var/datum/admins/D = new /datum/admins(rank, rights, ckey)
		D.associate(directory[ckey])

	world.log << "<b>Loaded admins.</b>"
...
/proc/log_admin_rank_modification(var/adm_ckey, var/new_rank)
	return //todo

/proc/log_admin_permission_modification(var/adm_ckey, var/new_perm)
	return //todo

/proc/edit_admin_permissions()
	return //todo
