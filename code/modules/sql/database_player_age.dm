...
/client/proc/log_client_to_db()
	...
	if(player_exists) //Player already identified previously, we need to just update the 'lastseen', 'ip' and 'computer_id' variables
		query_update = new("UPDATE player SET lastseen = datetime('now'), ip = ?, computerid = ?, lastadminrank = ? WHERE ckey = ?;", address, computer_id, admin_rank, ckey(key))
	else //New player!! Need to insert all the stuff
		query_update = new("INSERT INTO player VALUES (?, datetime('now'), datetime('now'), ?, ?, ?);", ckey(key), address, computer_id, admin_rank)
