...
/proc/jobban_loadbanfile()
	if(!config.sql_enabled)
		...
		//Job permabans
		var/database/query/query = new("SELECT ckey, job FROM ban WHERE bantype = 'JOB_PERMABAN' AND isnull(unbanned)")
		query.Execute(global_db)
		while(query.NextRow())
			var/list/querydata = query.GetRowData()
			jobban_keylist.Add("[querydata["ckey"]] - [querydata["job"]]")

		//Job tempbans
		var/database/query/query1 = new("SELECT ckey, job FROM ban WHERE bantype = 'JOB_TEMPBAN' AND isnull(unbanned) AND expiration_time > Now()")
		query1.Execute(global_db)
		while(query1.NextRow())
			var/list/querydata = query1.GetRowData()
			jobban_keylist.Add("[querydata["ckey"]] - [querydata["job"]]")
...
/datum/admins/proc/DB_ban_record(var/bantype, var/mob/banned_mob, var/duration = -1, var/reason, var/job = "", var/rounds = 0, var/banckey = null, var/banip = null, var/bancid = null)
	...
	var/database/query/query = new("SELECT * FROM player WHERE ckey = '[ckey]'")
	query.Execute(global_db)
	var/validckey = 0
	if(query.NextRow())
		validckey = 1
	...
	var/database/query/query_insert = new(
		"INSERT INTO ban VALUES (null, date('now'), '[serverip]', '[bantype_str]', '[reason]', '[job]', [(duration)?"[duration]":"0"], \
		[(rounds)?"[rounds]":"0"], date('now') + INTERVAL [(duration>0) ? duration : 0] MINUTE, '[ckey]', '[computerid]', '[ip]', '[a_ckey]', \
		'[a_computerid]', '[a_ip]', '[who]', '[adminwho]');"
		)
...
/datum/admins/proc/DB_ban_unban(var/ckey, var/bantype, var/job = "")
	...
	var/database/query/query = new(sql)
	query.Execute(global_db)
	while(query.NextRow())
		var/list/querydata = query.GetRowData()
		ban_id = querydata["id"]
...
/datum/admins/proc/DB_ban_edit(var/banid = null, var/param = null)
	...
	var/database/query/query = new("SELECT ckey, duration, reason FROM ban WHERE id = [banid]")
	query.Execute(global_db)
	...
	switch(param)
		if("reason")
			...
			var/database/query/update_query = new("UPDATE ban SET reason = '[value]', edits = CONCAT(edits,'- [eckey] changed ban reason from <cite><b>\\\"[reason]\\\"</b></cite> to <cite><b>\\\"[value]\\\"</b></cite><BR>') WHERE id = [banid]")
			update_query.Execute(global_db)
		...
		if("duration")
			...
			var/database/query/update_query = new("UPDATE ban SET duration = [value], edits = CONCAT(edits,'- [eckey] changed ban duration from [duration] to [value]<br>'), expiration_time = DATE_ADD(bantime, INTERVAL [value] MINUTE) WHERE id = [banid]")
...
/datum/admins/proc/DB_ban_unban_by_id(var/id)
	...
	var/database/query/query_update = new(sql_update)
...
/datum/admins/proc/DB_ban_panel(var/playerckey = null, var/adminckey = null, var/playerip = null, var/playercid = null, var/dbbantype = null, var/match = null)
	...
	if(adminckey || playerckey || playerip || playercid || dbbantype)
		...
		if(adminckey || playerckey || playerip || playercid || dbbantype)
			...
			var/database/query/select_query = new("SELECT id, bantime, bantype, reason, job, duration, expiration_time, ckey, a_ckey, unbanned, unbanned_ckey, unbanned_datetime, edits, ip, computerid FROM ban WHERE 1 [playersearch] [adminsearch] [ipsearch] [cidsearch] [bantypesearch] ORDER BY bantime DESC LIMIT 100")
			...
			while(select_query.NextRow())
				...
				var/banid =      querydata["id"]
				var/bantime =    querydata["bantime"]
				var/bantype =    querydata["bantype"]
				var/reason =     querydata["reason"]
				var/job =        querydata["job"]
				var/duration =   querydata["duration"]
				var/expiration = querydata["expiration_time"]
				var/ckey =       querydata["ckey"]
				var/ackey =      querydata["a_ckey"]
				var/unbanned =   querydata["unbanned"]
				var/unbanckey =  querydata["unbanned_ckey"]
				var/unbantime =  querydata["unbanned_datetime"]
				var/edits =      querydata["edits"]
				var/ip =         querydata["ip"]
				var/cid =        querydata["computerid"]
...
/proc/AddBan(var/ckey, var/cid, var/reason, var/bantype, var/arg1, var/arg2)
	return