...
/turf/proc/lighting_clear_overlay()
	if (lighting_overlay)
		if (lighting_overlay.loc != src)
			...
			crash_with("Lighting overlay variable on turf at ([x],[y],[z]) is insane, lighting overlay actually located at ([badT.x],[badT.y],[badT.z])!")
...
/turf/proc/generate_missing_corners()
	if (!TURF_IS_DYNAMICALLY_LIT_UNSAFE(src) && !light_sources && !(z_flags & ALLOW_LIGHTING))
		return
