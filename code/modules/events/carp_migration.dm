	...
	while (i <= num_groups)
		...
		if(prob(96))
			for (var/j = 1, j <= group_size, j++)
				var/critter = using_map.get_minor_critter(1)
				spawned_carp.Add(new critter(spawn_locations[i]))
			i++
		else
			group_size = max(1,round(group_size/6))
			group_size = min(spawn_locations.len-i+1,group_size)
			for(var/j = 1, j <= group_size, j++)
				var/critter = using_map.get_major_critter(1)
				spawned_carp.Add(new critter(spawn_locations[i+j]))
			i += group_size
