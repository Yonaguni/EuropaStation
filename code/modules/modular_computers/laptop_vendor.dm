...
/obj/machinery/lapvend/proc/fabricate_and_recalc_price(var/fabricate = 0)
	...
	if(devtype == 1) 		// Laptop, generally cheaper to make it accessible for most station roles
		...
		switch(dev_cpu)
			if(1)
				if(fabricate)
					fabricated_laptop.cpu.processor_unit = new/obj/item/computer_hardware/processor_unit/small(fabricated_laptop.cpu)
			if(2)
				if(fabricate)
					fabricated_laptop.cpu.processor_unit = new/obj/item/computer_hardware/processor_unit(fabricated_laptop.cpu)
				total_price += 299
		switch(dev_battery)
			if(1) // Basic(750C)
				if(fabricate)
					fabricated_laptop.cpu.battery_module = new/obj/item/computer_hardware/battery_module(fabricated_laptop.cpu)
			if(2) // Upgraded(1100C)
				if(fabricate)
					fabricated_laptop.cpu.battery_module = new/obj/item/computer_hardware/battery_module/advanced(fabricated_laptop.cpu)
				total_price += 199
			if(3) // Advanced(1500C)
				if(fabricate)
					fabricated_laptop.cpu.battery_module = new/obj/item/computer_hardware/battery_module/super(fabricated_laptop.cpu)
				total_price += 499
		switch(dev_disk)
			if(1) // Basic(128GQ)
				if(fabricate)
					fabricated_laptop.cpu.hard_drive = new/obj/item/computer_hardware/hard_drive(fabricated_laptop.cpu)
			if(2) // Upgraded(256GQ)
				if(fabricate)
					fabricated_laptop.cpu.hard_drive = new/obj/item/computer_hardware/hard_drive/advanced(fabricated_laptop.cpu)
				total_price += 99
			if(3) // Advanced(512GQ)
				if(fabricate)
					fabricated_laptop.cpu.hard_drive = new/obj/item/computer_hardware/hard_drive/super(fabricated_laptop.cpu)
				total_price += 299
		switch(dev_netcard)
			if(1) // Basic(Short-Range)
				if(fabricate)
					fabricated_laptop.cpu.network_card = new/obj/item/computer_hardware/network_card(fabricated_laptop.cpu)
				total_price += 99
			if(2) // Advanced (Long Range)
				if(fabricate)
					fabricated_laptop.cpu.network_card = new/obj/item/computer_hardware/network_card/advanced(fabricated_laptop.cpu)
				total_price += 299
		if(dev_tesla)
			total_price += 399
			if(fabricate)
		...
		if(dev_nanoprint)
			total_price += 99
			if(fabricate)
				fabricated_laptop.cpu.nano_printer = new/obj/item/computer_hardware/nano_printer(fabricated_laptop.cpu)
		if(dev_card)
			total_price += 199
			if(fabricate)
				fabricated_laptop.cpu.card_slot = new/obj/item/computer_hardware/card_slot(fabricated_laptop.cpu)
...
obj/machinery/lapvend/attackby(var/obj/item/W, var/mob/user)
