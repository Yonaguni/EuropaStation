#define STATION_DISTANCE_FROM_SUN 5.2

var/list/weighted_randomevent_locations = list()  // Assoc. list indexed by /datum/trade_destination instances.
var/list/weighted_mundaneevent_locations = list() // As above. Both use pickweight() and the size of their event lists.

/datum/trade_destination
	var/name = ""
	var/description = ""
	var/distance = 0                        // Assuming AU for the purposes of this lore. Currently unused.
	var/list/temp_price_change = list()     // Currently unused but modified by economic random events.
	var/list/viable_random_events = list()
	var/list/viable_mundane_events = list()

/datum/trade_destination/New()
	distance = abs(distance-STATION_DISTANCE_FROM_SUN)
	..()
