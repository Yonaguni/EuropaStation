	...
	if(!body)
		...
		switch(event_type)
			if(RESEARCH_BREAKTHROUGH)
				body = "A major breakthough in the field of [pick("material science research","super-compressed materials","nano-augmentation","bluespace research","volatile power manipulation")] \
			...
			if(MOVIE_RELEASE)
				...
				[pick("Deadly","The last","Lost","Dead")] [pick("Starships","Warriors","outcasts","Skrell")] \
			...
			if(GOSSIP)
				...
				if(prob(33))
					...
					"the decision to be dropped by a major sponsor","a disasterous interview on Sol Tonight")]."
			if(TOURISM)
...
/datum/event/trivial_news/announce()
	...
	"Most [affected_dest.name] police officers prefer cream over sugar.",\
	...
	"'Here kitty kitty' no longer preferred tiger retrieval technique.",\
	...
	"Spacer gets tattoo of Callisto on chest '[pick("[using_map.boss_short]","star","starship","asteroid")] tickles most'.",\
	...
	"Survey: 'Cheese Louise' Voted Best Pizza Restaurant In Sol",\
	...
	"Boy Saves Resomi From Tree on [affected_dest.name], Thousands Cheer",\
	"Shipment Of Apples Overturns, [affected_dest.name] Diner Offers Applesauce Special",\
	"Spotted Owl Spotted on [affected_dest.name]",\
	"Humans Everywhere Agree: Warbling Skrell Are Happy Skrell",\
