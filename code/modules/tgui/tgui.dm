...
/datum/tgui/proc/get_html(var/inline)
	...
	if(inline)
		html = replacetextEx(tguiProcess.basehtml, "{}", get_json(initial_data))
	else
		html = tguiProcess.basehtml
...
/datum/tgui/Topic(href, href_list)
	...
	switch(action)
		...
		if("tgui:view")
			...
			tguiProcess.update_uis(src_object)
		if("tgui:link")
		...
		if("tgui:nofrills")
			...
			if(src_object.ui_act(action, params, src, state)) // Call ui_act() on the src_object.
				tguiProcess.update_uis(src_object) // Update if the object requested it.
