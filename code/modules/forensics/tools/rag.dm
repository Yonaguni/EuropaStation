...
/obj/item/weapon/reagent_containers/glass/rag/proc/ignite()
	...
	if(reagents.get_reagent_amount(REAGENT_ID_FUEL)) // the phoron explodes when exposed to fire
		visible_message("<span class='danger'>\The [src] conflagrates violently!</span>")
		var/datum/effect/effect/system/reagents_explosion/e = new()
		e.set_up(round(reagents.get_reagent_amount(REAGENT_ID_FUEL) / 2.5, 1), get_turf(src), 0, 0)
		e.start()
		qdel(src)
		return
