...
/obj/machinery/fusion_fuel_compressor/proc/do_special_fuel_compression(var/obj/item/thing, var/mob/user)
	if(istype(thing) && thing.reagents && thing.reagents.total_volume && thing.is_open_container())
		if(thing.reagents.reagent_list.len > 1)
			user << "<span class='warning'>The contents of \the [thing] are impure and cannot be used as fuel.</span>"
			return 1
		if(thing.reagents.total_volume < 50)
			user << "<span class='warning'>You need at least fifty units of material to form a fuel rod.</span>"
			return 1
		...
		var/obj/item/fuel_assembly/F = new(get_turf(src), "exotic matter")
