...
/obj/item/psychic_power/telekinesis/proc/set_focus(var/atom/movable/_focus)
	...
	if(check_paramount && owner.psi.get_rank(PSI_PSYCHOKINESIS) < PSI_RANK_PARAMOUNT)
		to_chat(owner, "<span class='warning'>\The [_focus] is too hefty for you to get a mind-grip on.</span>")
		qdel(src)
		return FALSE
...
/obj/item/psychic_power/telekinesis/afterattack(var/atom/target, var/mob/living/user, var/proximity)
	...
	if(target == focus)
		...
		user.visible_message("<span class='danger'>\The [user] gestures sharply!</span>")
		sparkle()
...
/obj/item/psychic_power/telekinesis/proc/sparkle()
	set waitfor = 0
	if(focus)
		var/obj/effect/overlay/O = PoolOrNew(/obj/effect/overlay, get_turf(focus))
		O.name = "sparkles"
		O.anchored = 1
		O.density = 0
		O.layer = FLY_LAYER
