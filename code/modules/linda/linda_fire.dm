...
/turf/simulated/hotspot_expose(exposed_temperature, exposed_volume, soh)
	...
	if(active_hotspot)
		if(soh)
			if(air_contents.gas["phoron"] > 0.5 && air_contents.gas["oxygen"] > 0.5)
	...
	if((exposed_temperature > PHORON_MINIMUM_BURN_TEMPERATURE) && air_contents.gas["phoron"] > 0.5)
		igniting = 1

	if(igniting)
		if(air_contents.gas["oxygen"] < 0.5 || air_contents.gas["phoron"] < 0.5)
			return 0
...
/obj/effect/hotspot/process()
	...
	if(!(turf_air) || turf_air.gas["phoron"] < 0.5 || turf_air.gas["oxygen"] < 0.5)
		Kill()
		return
