...
/datum/shuttle/ferry/multidock/init_docking_controllers()
	if(docking_controller_tag_station)
		...
		if(!istype(docking_controller_station))
			log_debug("<span class='danger'>warning: shuttle with docking tag [docking_controller_station] could not find it's controller!</span>")
	if(docking_controller_tag_offsite)
		...
		if(!istype(docking_controller_offsite))
			log_debug("<span class='danger'>warning: shuttle with docking tag [docking_controller_offsite] could not find it's controller!</span>")
	if (!location)
