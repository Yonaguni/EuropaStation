...
/datum/antagonist/traitor
	id = MODE_TRAITOR
	protected_jobs = list("Security Officer", "Munitions Officer", "Chief of Security", "Commanding Officer")
...
/datum/antagonist/traitor/proc/spawn_uplink(var/mob/living/carbon/human/traitor_mob)
	...
	if(traitor_mob.client.prefs.uplinklocation == "Headset")
		...
		if(!R)
			R = locate(/obj/item/device/radio/headset/pda) in traitor_mob.contents
	...
	else if (istype(R, /obj/item/device/radio/headset/pda))
		// generate a passcode if the uplink is hidden in a PDA
		var/pda_pass = "[rand(100,999)] [pick("Alpha","Bravo","Delta","Omega")]"
		var/obj/item/device/uplink/T = new(R, traitor_mob.mind)
		R.hidden_uplink = T
		var/obj/item/device/radio/headset/pda/P = R
