...
/obj/item/camera_assembly/attackby(obj/item/W as obj, var/mob/living/user)
	...
	switch(state)
		...
		if(3)
			...
			if(W.isscrewdriver())
				...
				var/input = sanitize(input(usr, "Which networks would you like to connect this camera to? Separate networks with a comma. No Spaces!\nFor example: Exodus,Security,Secret", "Set Network", camera_network ? camera_network : NETWORK_GENERAL))
				if(!input)
...
/obj/item/camera_assembly/attack_hand(var/mob/user)
	if(!anchored)
		..()
