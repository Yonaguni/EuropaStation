...
/obj/item/airlock_brace/Destroy()
	for(var/obj/item/brace_keycard/C in keycards)
		C.brace = null
		keycards.Remove(C)
	keycards = null
	if(airlock)
...
/obj/item/airlock_brace/attackby(var/obj/item/W, var/mob/user)
	..()
	if (istype(W, /obj/item/brace_keycard))
		var/obj/item/brace_keycard/C = W
		if(!airlock)
			C.brace = src
			keycards |= C
