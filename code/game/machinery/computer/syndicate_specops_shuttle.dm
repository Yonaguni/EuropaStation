...
/obj/machinery/computer/syndicate_elite_shuttle/Topic(href, href_list)
	...
	if (href_list["sendtodock"])
		...
		usr << "<span class='notice'>The Syndicate will not allow the Elite Squad submarine to return.</span>"
		return
		...
		if (!specops_can_move())
			usr << "<span class='warning'>The Syndicate Elite submarine is unable to leave.</span>"
			return

		usr << "<span class='notice'>The Syndicate Elite submarine will arrive on [station_name] in [(SYNDICATE_ELITE_MOVETIME/10)] seconds.</span>"
