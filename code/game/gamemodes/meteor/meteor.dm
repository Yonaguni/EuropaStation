...
/datum/game_mode/meteor
	name = "Meteor"
	round_description = "The crew has been stuck in a major meteor shower."
	extended_round_description = "We are on an unavoidable collision course with an asteroid field, and will be continuously slammed with meteors, venting hallways, rooms, and ultimately destroying a majority of the basic life functions of the entire structure. Coordinate with your fellow crew members to survive the inevitable destruction and get back home in one piece!"
	config_tag = "meteor"
	required_players = 0
	votable = 0
	deny_respawn = 1
...
/datum/game_mode/meteor/declare_completion()
	...
	for(var/mob/living/player in player_list)
		if(player.stat != DEAD)
			...
			else if(location.loc.type == get_escape_shuttle_area())
