...
/proc/announce_hack_failure(var/mob/living/silicon/ai/user = null, var/text)
	...
	switch(user.hack_fails)
		...
		if(2)
			fulltext = "We have detected another hack attempt. It was targeting [text]. The intruder almost gained control of the system, so we had to disconnect them. We partially finished our trace and it seems to be originating either from [station_name()], or its immediate vicinity."
		if(3)
