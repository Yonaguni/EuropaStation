...
/obj/item/weapon/cartridge/proc/create_NanoUI_values(mob/user as mob)
	...
	if(mode==49)
		...
		var/MopData[0]
		for(var/obj/item/weapon/mop/M in all_structures)
		...
		var/BucketData[0]
		for(var/obj/structure/mopbucket/B in all_structures)
		...
		var/CartData[0]
		for(var/obj/structure/janitorialcart/B in all_structures)
			var/turf/bl = get_turf(B)
			if(bl)
				if(bl.z != cl.z)
					continue
