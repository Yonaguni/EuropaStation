...
/datum/effect/system/foam_spread/set_up(var/n = 5, var/c = 0, var/loc, var/datum/reagents/carry = null, var/metalfoam = 0)
	..()
	if(!location)
		return
	amount = round(sqrt(number/ 3), 1)
	...
	if(carry && !metal)
		for(var/rid in carry.reagent_list)
			carried_reagents += rid
...
/datum/effect/system/foam_spread/start()
	spawn(0)
		...
		F = PoolOrNew(/obj/effect/foam, list(location, metal))
		F.amount = amount
