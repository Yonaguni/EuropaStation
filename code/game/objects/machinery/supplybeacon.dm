...
/obj/item/supply_beacon
	...
	desc = "An inactive, hacked supply beacon stamped with the Jovian Rapid Fabrication logo. Good for one (1) ballistic supply pod shipment."
	icon_state = "beacon"
...
/obj/machinery/power/supply_beacon/process()
	...
	if(!target_drop_time)
		...
		var/drop_y = src.y-2
		var/drop_z = src.z
		command_announcement.Announce("Jovian Rapid Fabrication priority supply request #[rand(1000,9999)]-[rand(100,999)] recieved. Shipment dispatched via ballistic supply pod for immediate delivery. Have a nice day.", "Thank You For Your Patronage")
		spawn(rand(100,300))
