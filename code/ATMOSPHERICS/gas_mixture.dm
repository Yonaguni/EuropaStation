...
/datum/gas_mixture/proc/adjust_gas_temp(gasid, moles, temp, update = 1)
	...
	if(moles > 0 && abs(temperature - temp) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
		...
		var/giver_heat_capacity = SSmaterials.specific_heats[gasid] * moles
...
/datum/gas_mixture/proc/heat_capacity()
	. = 0
	for(var/g in gas)
		. += SSmaterials.specific_heats[g] * gas[g]
	. *= group_multiplier
...
/datum/gas_mixture/proc/specific_entropy_gas(var/gasid)
	...
	var/molar_mass = SSmaterials.molar_masses[gasid]
	var/specific_heat = SSmaterials.specific_heats[gasid]
...
/datum/gas_mixture/proc/remove_by_flag(flag, amount)
	...
	var/sum = 0
	for(var/g in gas)
		if(SSmaterials.gas_flags[g] & flag)
			sum += gas[g]
...
/datum/gas_mixture/proc/check_tile_graphic(list/graphic_add = null, list/graphic_remove = null)
	for(var/g in SSmaterials.gas_overlay_limits)
		if(graphic.Find(SSmaterials.gas_tile_overlays[g]))
			//Overlay is already applied for this gas, check if it's still valid.
			if(gas[g] <= SSmaterials.gas_overlay_limits[g])
				if(!graphic_remove)
					graphic_remove = list()
				graphic_remove += SSmaterials.gas_tile_overlays[g]
		else
			//Overlay isn't applied for this gas, check if it's valid and needs to be added.
			if(gas[g] > SSmaterials.gas_overlay_limits[g])
				if(!graphic_add)
					graphic_add = list()
				graphic_add += SSmaterials.gas_tile_overlays[g]
...
/datum/gas_mixture/proc/get_mass()
	for(var/g in gas)
		. += gas[g] * SSmaterials.molar_masses[g] * group_multiplier
