...
/obj/machinery/atmospherics/omni/filter/error_check()
	if(!input || !output || !filter_values)
		return 1
	if(filter_values.len < 1) //requires at least 1 filter ~otherwise why are you using a filter?
		return 1
...
/obj/machinery/atmospherics/omni/filter/process()
	...
	if (power_draw >= 0)
		...
		for(var/datum/omni_port/P in filter_values)
			if(P.network)
				P.network.update = 1
...
/obj/machinery/atmospherics/omni/filter/Topic(href, href_list)
	...
	if(configuring && !use_power)
		switch(href_list["command"])
			...
			if("switch_filter")
				var/new_filter = input(usr,"Select filter mode:","Change filter",href_list["mode"]) in list("None", "Oxygen", "Nitrogen", "Carbon Dioxide", "Fuel", "Nitrous Oxide", "Hydrogen")
